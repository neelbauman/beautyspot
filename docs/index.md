...
### 🧹 4. Automated Garbage Collection (Auto-Eviction)

長期間稼働するバッチ処理やWebサーバーにおいて、ストレージの無限の肥大化を防ぐために **「確率的自動エビクション（Probabilistic Auto-Eviction）」** を搭載しています。
メインスレッドのレイテンシを一切犠牲にすることなく、TTL（有効期限）切れのデータや孤立したBlobファイルをバックグラウンドで自動的に削除します。

```python
# キャッシュミス（新規保存）が発生するたびに、1%の確率で自動的にゴミ掃除を行う
...
### 🧩 5. Dependency Injection (Custom Backend)
