...
## ストレージポリシー (`StoragePolicyProtocol`)

データのサイズや性質に応じて、外部ストレージ（Blob）に保存するか、データベース内に直接保存するかを動的に決定するためのポリシーです。`Spot` の初期化時に `storage_policy` として指定できます。

* **`ThresholdStoragePolicy(threshold_bytes)`**: 指定したバイト数（デフォルトは1MB等）を超えるデータのみを外部ストレージに保存し、小さなデータはDB内に直接保存します。
* **`AlwaysBlobPolicy()`**: データのサイズに関わらず、すべてのデータを外部ストレージに保存します（デフォルトの動作）。
* **`WarningOnlyPolicy(threshold_bytes)`**: 基本的にDB内に保存しますが、指定サイズを超えた場合に警告（Warning）を出力します。
...
* **クリーンアップ**:
    * **`clean_temp_files(max_age_seconds)`**: クラッシュ等で残存した一時ファイル（`.tmp`）を削除します。
    * **`prune_empty_dirs()`**: データが空になった不要なディレクトリツリーを削除します。
