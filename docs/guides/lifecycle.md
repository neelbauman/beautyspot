...
```
ご提示した `lifecycle.md` に、今回追加した「自動エビクション機能」の説明を統合した完全版のマークダウンを作成しました。

このドキュメントでは、既存の「手動GC（Garbage Collection）」の解説を残しつつ、新機能である「自動エビクション」の章を追加し、ユーザーが用途に合わせて使い分けられるように構成しています。
