...
github-release: check-version test build  ## GitHubリリース作成（タグをpush）
	@if [ -z "$(VERSION)" ]; then \
		echo "Error: Could not read version from pyproject.toml"; \
		exit 1; \
	fi
...
pypi-publish: test build  ## PyPIにローカルから公開
	@if [ -z "$(VERSION)" ]; then \
		echo "Error: Could not read version from pyproject.toml"; \
		exit 1; \
	fi
...
release: github-release pypi-publish  ## 完全リリース（GitHubタグ作成→PyPI公開）
