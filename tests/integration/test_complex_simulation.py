...
def chaos_env(tmp_path):
    ...
    # カスタム型の登録
    spot.register_type(
        Complexity, code=10, encoder=encode_complexity, decoder=decode_complexity
    )
...
async def test_resilient_pipeline_simulation(chaos_env):
    ...
    # 実行追跡用
    stats = {"process_runs": 0, "failures": 0}
    ...
    results_ph2 = await asyncio.gather(
        *tasks_retry, return_exceptions=False
    )  # 今度は例外なし
    ...
    assert stats["process_runs"] == 11, (
        "キャッシュ済みのタスクが再実行されてしまっています"
    )
