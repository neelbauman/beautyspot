...
def test_storage_failure_handling(tmp_path):
    ...
    project = Project(name="test_proj", db=str(tmp_path / "test.db"), storage_path=str(tmp_path / "blobs"))
...
def test_invalid_json_serialization(tmp_path):
    """Test task with non-serializable return value when save_blob=False."""
    project = Project(name="test_proj", db=str(tmp_path / "test.db"))
    ...
    # First run: returns original object
    res1 = bad_task()
    assert isinstance(res1, Unserializable)
    
    # Second run: returns cached string (because it was saved as string in DB)
    res2 = bad_task()
    assert isinstance(res2, str)
    assert "Unserializable object" in res2
